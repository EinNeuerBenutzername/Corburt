...
struct et_room{
    int id;
    int money;
    int etenemy[DBE_ENEMYCAP];
    int etitem[DBE_ITEMCAP];
    int etinteract[DBE_INTERCAP];
};
...
struct et_room *et_findroomwithid(int roomid);
void etenemy_expand();
void etenemy_push(int enemyid, int roomid);
void etenemy_attack(int etid, struct et_room *etr);
void etenemy_takedamage(int entityid, int dmg);
void etenemy_die(int entityid);
void etenemy_deathdrops(enemydb *edb, int roomid);
void etenemy_list();
enemydb *et_getenemydb(int enemyentityid);
void etitem_expand();
void etitem_delete(int entityid);
void etitem_rebind(int entityid, int newroomid);
void etitem_push(int itemid, int qnty, int roomid, int purchase);
itemdb *et_getitemdb(int itementityid);
void et_initenemies();
...
void etenemy_expand(){
    ...
    struct et_enemy ete={0};
    for(int i=enemiesmax-DBE_EXPANDSTEP; i<enemiesmax; i++){
        et_enemies[i]=ete;
    }
}
void etenemy_push(int enemyid, int roomid){
    ...
    enemydb *edb=NULL;
    int enemyreferenceindex=-1, // spawn in etr->etenemy[index]
    ...
    for(int i=enemypointer; i<enemiesmax; i++){
...
void etenemy_attack(int etid, struct et_room *etr){
    ...
    if(pcandodge(edb)){
        ...
        int atk=edb->stats.atk;
        int dmgcolor=palette.value.plain;
...
void etenemy_deathdrops(enemydb *edb, int roomid){
    ...
    { // money drops
        int money=edb->loot.moneymin;
        int delta=edb->loot.moneymax-edb->loot.moneymin;
        if(delta<0)delta=0-delta;
        if(delta>0){
...
//        }
    ...
    if(edb->loot.drops[0].itemid){ // item drops
        for(int i=0; i<ENEMY_MAXDROPS; i++){
            if(edb->loot.drops[i].itemid==0)break;
            if(genRandLong(&mtrand)%10000<(unsigned int)edb->loot.drops[i].rate){
...
void etitem_expand(){
    ...
    struct et_item eti={0};
    for(int i=itemsmax-DBE_EXPANDSTEP; i<itemsmax; i++){
        et_items[i]=eti;
    }
}
void etitem_delete(int entityid){
...
void etitem_push(int itemid, int qnty, int roomid, int purchase){
    if(qnty<=0)return; // an error has occurred, but I'll leave it alone
    bool success=false;
    struct et_room *etr=NULL;
    itemdb *idb=NULL;
    int itemreferenceindex=-1, // spawn in etr->etitem[index]
    ...
    int stackable=idb->type&db_itemtype_stackable_mask;
    for(int i=itempointer; i<itemsmax; i++){
    ...
    if(roomid!=0){ // add into room
        ...
        if(!stackable){
            ...
            bool hasslot=false;
            if(qnty>ITEM_MAXSTACK)qnty=ITEM_MAXSTACK;
            for(int i=0; i<inventory.unlocked && qnty>0; i++){
...
void et_initenemies(){
    for(int i=0; i<roomdbsize; i++){
        ...
        int roomenemytablesize=0;
        for(int j=0;roomdbs[i].table[j]!=0; j++)roomenemytablesize++;
        for(int j=0, c=0; j<DBE_ENEMYCAP; j++){
            if(etr->etenemy[j]!=0)c++;
            if(j==DBE_ENEMYCAP-1 && c>=roomenemytablesize)return;
        }
        for(int j=0; j<roomenemytablesize; j++){
