...
bool matchcommands(char *cmd);
bool match__commands(char *cmd); // template
bool matchregularcommands(char *cmd);
bool matcheditstatscommands(char *cmd);
bool matchyesnocommands(char *cmd);
void processinput();
#ifdef CB_REALTIME
#include "cbsys.h"
...
bool matchregularcommands(char *cmd){
    ...
    if(fullmatch(cmd, "use")){
        ...
        for(int i=0; i<inventory.unlocked; i++){
            ...
            int curmatch=match(usetarget, idb->name);
            if(curmatch>maxmatch){
                noprev=false;
    ...
    if(fullmatch(cmd, "take") ||
       ...
       fullmatch(cmd, "g")){
        ...
        for(int i=0; i<DBE_ITEMCAP; i++){
            ...
            int curmatch=match(taketarget, idb->name);
            if(curmatch>maxmatch){
                maxmatch=curmatch;
    ...
    if(fullmatch(cmd, "drop")){
        ...
        for(int i=0; i<inventory.unlocked; i++){
            ...
            int curmatch=match(droptarget, idb->name);
            if(curmatch>maxmatch){
                maxmatch=curmatch;
    ...
    if(fullmatch(cmd, "sell")){
        ...
        for(int i=0;rm->table[i]!=0; i++){
            ...
            int curmatch=match(selltarget, idb->name);
            if(curmatch>maxmatch){
                maxmatch=curmatch;
                maxmatchitemid=idb->id;
            }else if(curmatch==maxmatch){
                for(int k=0; k<inventory.unlocked; k++){
    ...
    if(fullmatch(cmd, "attack") ||
       fullmatch(cmd, "a")){
        ...
        struct et_room *etr=et_findroomwithid(rm->id);
        int maxmatch=-1;
//        int maxmatchenemyid=0;
        int maxmatchenemyentityid=0;
        for(int i=0; i<DBE_ENEMYCAP; i++){
            ...
            int curmatch=match(attacktarget, edb->name);
            if(curmatch>maxmatch){
                maxmatch=curmatch;
