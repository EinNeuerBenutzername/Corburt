...
roomdb *db_rfindwithid(int roomid);
void db_rshowdesc(int roomid);
void db_rshowtable(int roomid);
regiondb *db_rgfindwithregion(int region);
void db_rgshowdesc(int region);
roomdb *db_rfindwithid(int roomid){
...
void db_rshowdesc(int roomid){
    ...
    if(etr==NULL){
        ...
        for(int i=0, first=1; i<DBE_ENEMYCAP; i++){ // enemies:
            if(etr->etenemy[i]!=0){
        ...
        for(int i=0, first=1; i<DBE_ITEMCAP; i++){ // items:
            if(etr->etitem[i]!=0){
