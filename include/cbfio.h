...
#include "cbbase.h"
#include <stdlib.h>
static void readsave(int saveid, FILE *fp);
static void savesave(int saveid, FILE *fp);
static void save_overwrite(int saveid);
static void save_overwritten(int saveid);
...
static void saveet_items(FILE *fp){
    ...
    for(int i=0; i<itemsmax; i++){
        int avail=et_items[i].available?1:0;
...
//    tracelog("max enemies: %" PRIdFAST32 ".\n", enemiesmax_tmp);
    ...
    for(int i=0; i<enemiesmax_tmp; i++){
        int avail;
        int_fast32_t_read(&avail, fp);
...
static void saveet_enemies(FILE *fp){
    ...
    for(int i=0; i<enemiesmax; i++){
        int avail=et_enemies[i].available?1:0;
        int_fast32_t_write(avail, fp);
...
static void readet_rooms(FILE *fp){
    ...
    for(int i=0; i<roomsmax_tmp; i++){
        ...
        for(int j=0; j<DBE_ITEMCAP; j++)
            int_fast32_t_read(&et_rooms[i].etitem[j], fp);
        for(int j=0; j<DBE_INTERCAP; j++)
...
void readsaves(){
    ...
    if(fp_save==NULL){
        ...
        if(foundsaves==0)printr(palette.inform, msg->global_saveempty);
            ...
            for(int i=1; i<foundsaves; i++)printr(palette.inform, ", %s\n", saves[i].plr.name);
        }
    }
    while(true){
